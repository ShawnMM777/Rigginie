  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="fixed bottom-15 right-6 z-50 flex flex-col items-end gap-6">
        ...
        <button onClick={() => setOpen(!open)} className={`w-16 h-16 rounded-full border-4 border-gray-700 bg-white flex items-center justify-center shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-105 ${open ? 'rotate-45 bg-gray-100' : ''}`}>
...
function ProductCard({ product, onCartUpdated, onError }) {
    ...
    const navigate = useNavigate();
    const img = product.image ? `http://localhost:8000${product.image}` : null;
...
export default GRAPHICSCARDCATEGORIES
