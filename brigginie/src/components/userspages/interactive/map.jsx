...
import {
  Search, MapPin, Star, Navigation2, Heart, Share2,
  Plus, Minus, X, Clock, Layers, Route, Car, Bike, PersonStanding,
  ChevronLeft, Wifi, Camera, Moon, Sun, LocateFixed, List, LayoutGrid,
  ArrowUpDown, ChevronRight, Flame, BadgeCheck, CheckCircle2, Monitor, Cpu,
} from "lucide-react";
...
const DEFAULT_PLACES = [
  {
    id: 1, name: "Rigginie Factory Pasig Main Branch",
    lat: 14.58691, lng: 121.06140, rating: 4.9, reviews: 1420,
    open: true, dist: null, time: null, verified: true,
  ...
  {
    id: 2, name: "Rigginie Quezon City Branch",
    lat: 14.717, lng: 121.059, rating: 4.7, reviews: 882,
    open: true, dist: null, time: null, verified: true,
  ...
  {
    id: 3, name: "Rigginie Cavite Branch",
    lat: 14.5490, lng: 121.0310, rating: 4.8, reviews: 2100,
    open: true, dist: null, time: null, verified: true,
  ...
  {
    id: 4, name: "Rigginie Taguig Branch",
    lat: 14.5580, lng: 121.0160, rating: 4.9, reviews: 3240,
    open: true, dist: null, time: null, verified: false,
  ...
  {
    id: 5, name: "Rigginie Pasay Branch",
    lat: 14.5530, lng: 121.0350, rating: 4.8, reviews: 560,
    open: false, dist: null, time: null, verified: true,
  ...
  {
    id: 6, name: "Rigginie Cebu Branch",
    lat: 10.3167, lng: 123.8907, rating: 4.5, reviews: 430,
    open: true, dist: null, time: null, verified: false,
  ...
  {
    id: 7, name: "Rigginie Marikina Branch",
    lat: 14.5540, lng: 121.0190, rating: 4.4, reviews: 1870,
    open: true, dist: null, time: null, verified: true,
  ...
  {
    id: 8, name: "Rigginie Makati Branch",
    lat: 14.5600, lng: 121.0220, rating: 4.6, reviews: 980,
    open: true, dist: null, time: null, verified: false,
  ...
  {
    id: 9, name: "Rigginie Manila Branch",
    lat: 14.5515, lng: 121.0260, rating: 4.6, reviews: 740,
    open: true, dist: null, time: null, verified: true,
  ...
  {
    id: 10, name: "Rigginie Malabon Branch",
    lat: 14.5490, lng: 121.0290, rating: 4.8, reviews: 1120,
    open: true, dist: null, time: null, verified: true,
    tags: ["PC Rental", "Gaming", "Streaming"],
  ...
  {
    id: 11, name: "Rigginie Parañaque Branch",
    lat: 14.5565, lng: 121.0230, rating: 4.5, reviews: 388,
    open: false, dist: null, time: null, verified: false,
    tags: ["PC Rental", "Gaming"],
...
const PIN = {
  open:   { light: "bg-green-50 text-green-600",  badge: "bg-green-100 text-green-700" },
  closed: { light: "bg-red-50 text-red-600",      badge: "bg-red-100 text-red-700"     },
};
...
function makePinIcon(color, isSelected = false) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 26 34"><path d="M13 0C5.82 0 0 5.82 0 13c0 9.75 13 21 13 21S26 22.75 26 13C26 5.82 20.18 0 13 0z" fill="${color}" stroke="rgba(255,255,255,0.4)" stroke-width="1"/><circle cx="13" cy="13" r="6" fill="rgba(255,255,255,0.9)"/></svg>`;
  return L.divIcon({
    html: svg, className: "",
    iconSize: [w, h], iconAnchor: [w / 2, h],
    popupAnchor: [0, -h], tooltipAnchor: [w / 2, -h],
  });
}
...
function LeafletMap({ dark, places, selected, onSelect, mapRef, userLocation, nearMeActive }) {
  const youAreHereIcon = L.divIcon({
    html: `<div style="position:relative;width:24px;height:24px;"><div style="position:absolute;inset:0;border-radius:50%;background:rgba(99,102,241,0.25);animation:nearMePulse 1.8s ease-out infinite;"></div><div style="position:absolute;inset:4px;border-radius:50%;background:#6366f1;border:2px solid white;box-shadow:0 2px 8px rgba(99,102,241,0.5);"></div></div>`,
    className: "", iconSize: [24, 24], iconAnchor: [12, 12],
  });

  return (
    <MapContainer
      ref={mapRef} center={[14.5547, 121.0244]} zoom={15}
          ...
          >
            <Tooltip direction="top" offset={[0, -28]} opacity={1} className="leaflet-tooltip-custom">
              <div style={{ fontFamily: "system-ui,sans-serif" }}>
                ...
                {p.dist && <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 1 }}>{p.dist} away</div>}
              </div>
            </Tooltip>
          </Marker>
        );
...
function PlaceCard({ p, selected, dark, onClick }) {
  ...
  return (
    <button onClick={onClick}
      className={`group w-full text-left rounded-2xl p-3 border transition-all duration-150 ${isS
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${p.open ? "bg-green-100 text-green-700" : dark ? "bg-slate-800 text-slate-500" : "bg-slate-100 text-slate-500"}`}>
              {p.open ? "Open" : "Closed"}
            </span>
            {p.dist && <span className={`text-xs ${dark ? "text-slate-500" : "text-slate-400"}`}>{p.dist}</span>}
          </div>
          <div className="mt-2">
            ...
            <div className={`h-1.5 w-full rounded-full overflow-hidden ${dark ? "bg-slate-700" : "bg-slate-200"}`}>
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${pcPercent}%`, background: pcPercent === 0 ? "#ef4444" : pcPercent < 30 ? "#f97316" : "#22c55e" }} />
            </div>
          </div>
          {topGpu && (
            <div className="mt-1.5 flex items-center gap-1">
              <Cpu size={10} className="text-indigo-500" />
              <span className={`text-xs ${dark ? "text-slate-400" : "text-slate-500"}`}>{topGpu.name} · {topGpu.available} avail.</span>
            </div>
          )}
        </div>
...
function DetailDrawer({ p, dark, onClose }) {
  ...
  const pcPercent      = Math.round((p.availablePCs / p.totalPCs) * 100);
  const totalFreeGpus  = p.gpus.reduce((sum, g) => sum + g.available, 0);

  const handleShare = async () => {
    if (navigator.share) {
      try { await navigator.share({ title: p.name, text: p.desc, url: window.location.href }); }
      catch { /* cancelled */ }
    } else {
  ...
  return (
    <div className={`absolute right-0 top-0 bottom-0 w-80 z-[1000] shadow-2xl flex flex-col ${dark ? "bg-slate-900 border-l border-slate-800" : "bg-white border-l border-slate-100"}`}
      ...
      <div className={`relative h-44 shrink-0 ${col.light} flex items-center justify-center overflow-hidden`}>
        ...
        <button onClick={onClose}
          className={`absolute top-3 left-3 flex h-8 w-8 items-center justify-center rounded-full shadow-md hover:scale-105 transition-transform ${dark ? "bg-slate-900/80 text-slate-300" : "bg-white/90 text-slate-600"}`}>
          <ChevronLeft size={16} />
        </button>
        <button onClick={() => setSaved(v => !v)}
          className={`absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full shadow-md hover:scale-105 transition-all ${saved ? "bg-red-500 text-white" : "bg-white/90 text-slate-500"}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className={`px-4 pt-4 pb-3 border-b ${dark ? "border-slate-800" : "border-slate-100"}`}>
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={12} className={i < Math.floor(p.rating) ? "fill-amber-400 text-amber-400" : dark ? "text-slate-700" : "text-slate-200"} />
              ))}
              <span className={`ml-1 text-sm font-bold ${dark ? "text-slate-200" : "text-slate-700"}`}>{p.rating}</span>
            </div>
            <span className={`text-sm ${dark ? "text-slate-500" : "text-slate-400"}`}>{p.reviews.toLocaleString()} reviews</span>
          </div>
        </div>
        ...
        {/* PC availability */}
        <div className={`px-4 py-3 border-b ${dark ? "border-slate-800" : "border-slate-100"}`}>
          <p className={`mb-2 text-xs font-bold uppercase tracking-widest ${dark ? "text-slate-600" : "text-slate-400"}`}>PC Availability</p>
          <div className="flex items-center justify-between mb-2">
            <span className={`text-sm font-bold ${dark ? "text-slate-200" : "text-slate-800"}`}>{p.availablePCs} / {p.totalPCs} available</span>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${p.availablePCs === 0 ? "bg-red-100 text-red-600" : p.availablePCs < p.totalPCs * 0.3 ? "bg-amber-100 text-amber-600" : "bg-green-100 text-green-600"}`}>
        ...
        <div className={`px-4 py-3 border-b ${dark ? "border-slate-800" : "border-slate-100"}`}>
          <div className="flex items-center justify-between mb-2">
            <p className={`text-xs font-bold uppercase tracking-widest ${dark ? "text-slate-600" : "text-slate-400"}`}>Available GPUs</p>
            <span className={`text-xs font-semibold ${dark ? "text-slate-400" : "text-slate-500"}`}>{totalFreeGpus} total free</span>
          </div>
              ...
              return (
                <div key={g.name}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs ${g.available === 0 ? "text-red-400" : dark ? "text-slate-400" : "text-slate-500"}`}>{g.available}/{g.total}</span>
                  </div>
                  <div className={`h-1.5 w-full rounded-full overflow-hidden ${dark ? "bg-slate-700" : "bg-slate-200"}`}>
                    <div className="h-full rounded-full" style={{ width: `${pct}%`, background: g.available === 0 ? "#ef4444" : "#6366f1" }} />
                  </div>
                </div>
              );
            })}
        ...
        <div className={`px-4 py-3 border-b ${dark ? "border-slate-800" : "border-slate-100"}`}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {p.tags.map(t => (
              <span key={t} className={`rounded-full border px-3 py-1 text-xs font-medium ${dark ? "border-slate-700 text-slate-400 bg-slate-800" : "border-slate-200 text-slate-600 bg-slate-50"}`}>
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="px-4 py-3">
          ...
          <div className={`flex gap-1.5 rounded-xl border p-1 ${dark ? "border-slate-800 bg-slate-800/50" : "border-slate-200 bg-slate-50"}`}>
            {TRAVEL.map(({ key, Icon, label }) => (
              <button key={key} onClick={() => setTravel(key)}
                className={`flex flex-1 flex-col items-center gap-0.5 rounded-lg py-2 transition-all duration-150 ${travel === key ? "bg-indigo-600 text-white shadow-sm" : dark ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700"}`}>
                <Icon size={15} />
                <span className="text-xs font-semibold">{label}</span>
              </button>
            ))}
          </div>
          <div className={`mt-2 flex items-center justify-between rounded-xl px-3 py-2.5 ${dark ? "bg-slate-800" : "bg-slate-100"}`}>
      ...
      <div className={`shrink-0 px-4 py-3 border-t ${dark ? "border-slate-800" : "border-slate-100"}`}>
        <a
          ...
          className="w-full flex items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white hover:bg-indigo-700 active:scale-95 transition-all"
        >
          <Navigation2 size={15} /> Start Navigation
        </a>
        <button
          onClick={handleShare}
          className={`mt-2 w-full flex items-center justify-center gap-2 rounded-xl border py-3 text-sm font-semibold transition-all active:scale-95 ${dark ? "border-slate-700 text-slate-300 hover:bg-slate-800" : "border-slate-200 text-slate-700 hover:bg-slate-50"}`}
        >
...
function Map() {
  ...
  const placesWithDist = useMemo(() => {
    ...
    return places.map(p => {
      ...
      return { ...p, dist: formatDist(km), time: walkTime(km), _km: km };
    });
  }, [places, userLocation]);

  const filtered = useMemo(() => {
    let list = placesWithDist;
    if (catFilter === "open")   list = list.filter(p => p.open);
    if (catFilter === "closed") list = list.filter(p => !p.open);
    if (search.trim()) {
    ...
    if (nearMeActive && userLocation) {
      list = list
        .filter(p => (p._km ?? Infinity) <= NEAR_ME_RADIUS_KM)
        .sort((a, b) => (a._km ?? 0) - (b._km ?? 0));
    }
    return list;
  }, [catFilter, search, nearMeActive, userLocation, placesWithDist]);
  ...
  function handleNearMe() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setLocating(false);
        alert(err.code === 1 ? "Location access denied." : "Could not get your location.");
      },
  ...
  return (
    <div className={`relative flex h-screen w-full overflow-hidden select-none ${d ? "bg-slate-950" : "bg-slate-100"}`}
      style={{ fontFamily: "system-ui,-apple-system,sans-serif" }}>

      {/* Sidebar */}
      {showSide && (
        <aside className={`relative z-[500] flex w-[300px] shrink-0 flex-col ${d ? "bg-slate-900/95 border-r border-slate-800" : "bg-white/95 border-r border-slate-100"}`}
          ...
          <div className={`flex items-center gap-2.5 border-b px-4 py-3 ${d ? "border-slate-800" : "border-slate-100"}`}>
            ...
            <div className="flex h-8 w-8 items-center justify-center rounded-xl shadow-lg shadow-indigo-500/30">
              <img src={logo} alt="Rigginie" className="h-full w-full object-contain" />
            </div>
            <div>
            ...
            <button onClick={() => setShowSide(false)}
              className={`ml-auto rounded-lg p-1.5 transition-colors ${d ? "text-slate-500 hover:bg-slate-800 hover:text-slate-300" : "text-slate-400 hover:bg-slate-100 hover:text-slate-600"}`}>
              <ChevronLeft size={15} />
            </button>
          </div>
          ...
          <div className={`border-b px-3 py-3 ${d ? "border-slate-800" : "border-slate-100"}`}>
            <div className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 transition-colors ${d ? "border-slate-700 bg-slate-800 focus-within:border-indigo-500" : "border-slate-200 bg-slate-50 focus-within:border-indigo-400 focus-within:bg-white"}`}>
              <Search size={14} className={d ? "text-slate-500" : "text-slate-400"} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search branch, GPU…"
                className={`w-full bg-transparent text-sm outline-none ${d ? "text-slate-200 placeholder:text-slate-600" : "text-slate-700 placeholder:text-slate-400"}`} />
              {search && (
                <button onClick={() => setSearch("")} className={d ? "text-slate-500 hover:text-slate-300" : "text-slate-400 hover:text-slate-600"}>
                  <X size={13} />
                </button>
              )}
            </div>
            ...
            <div className="flex gap-1.5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
              {CATEGORIES.map(({ key, label, Icon }) => (
                <button key={key} onClick={() => setCatFilter(key)}
                  className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold transition-all ${catFilter === key ? "bg-indigo-600 text-white shadow-sm shadow-indigo-500/25" : d ? "bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                  <Icon size={11} />{label}
                </button>
              ))}
            </div>
          ...
          <div className={`flex items-center justify-between border-b px-3 py-2 ${d ? "border-slate-800" : "border-slate-100"}`}>
            ...
            <div className="flex items-center gap-1">
              <button className={`flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold transition-colors ${d ? "text-slate-400 hover:bg-slate-800 hover:text-slate-200" : "text-slate-500 hover:bg-slate-100"}`}>
              ...
              {[{ v: "list", I: List }, { v: "grid", I: LayoutGrid }].map(({ v, I }) => (
                <button key={v} onClick={() => setView(v)}
                  className={`rounded-lg p-1.5 transition-colors ${view === v ? d ? "bg-slate-700 text-slate-200" : "bg-slate-200 text-slate-700" : d ? "text-slate-600 hover:bg-slate-800" : "text-slate-400 hover:bg-slate-100"}`}>
                  <I size={13} />
                </button>
              ))}
            </div>
          ...
          <div className={`border-t px-4 py-3 flex items-center gap-2 ${d ? "border-slate-800" : "border-slate-100"}`}>
            ...
            {nearMeActive ? (
              <button onClick={handleClearNearMe}
                className="flex items-center gap-1.5 rounded-xl border border-indigo-400 bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-700 transition-colors">
                <X size={11} /> Clear
              </button>
            ) : (
              <button onClick={handleNearMe} disabled={locating}
                className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold transition-colors disabled:opacity-50 ${d ? "border-slate-700 text-slate-300 hover:bg-slate-800" : "border-slate-200 text-slate-700 hover:bg-slate-100"}`}>
                {locating
      ...
      <div className="relative flex-1 z-0" onClick={() => setSelected(null)}>
        <LeafletMap
          dark={d} places={filtered} selected={selected}
          onSelect={p => setSelected(prev => prev?.id === p.id ? null : p)}
          mapRef={mapRef} userLocation={userLocation} nearMeActive={nearMeActive}
        />
        ...
        <div className="absolute top-4 left-4 right-4 z-[500] flex items-start justify-between pointer-events-none">
          {!showSide ? (
            <button onClick={() => setShowSide(true)} style={{ pointerEvents: "auto" }}
              className={`flex h-10 w-10 items-center justify-center rounded-xl shadow-lg transition-all hover:scale-105 ${d ? "bg-slate-900/90 border border-slate-700 text-slate-300" : "bg-white/95 border border-slate-200 text-slate-600"}`}>
              <Monitor size={16} />
            </button>
          ) : <div />}

          <div className="flex flex-col gap-2 items-end" style={{ pointerEvents: "auto" }}>
            <div className={`flex items-center gap-2 rounded-2xl px-3 py-2 shadow-xl ${d ? "bg-slate-900/90 border border-slate-800" : "bg-white/95 border border-slate-100"}`}>
              <Wifi size={12} className="text-green-500" />
              <div className={`h-3 w-px mx-1 ${d ? "bg-slate-700" : "bg-slate-200"}`} />
              <button onClick={() => setDark(v => !v)}
                className={`flex h-6 w-6 items-center justify-center rounded-lg transition-colors ${d ? "text-amber-400 hover:bg-slate-800" : "text-indigo-500 hover:bg-slate-100"}`}>
                {d ? <Sun size={13} /> : <Moon size={13} />}
              </button>
              <button className={`flex h-6 w-6 items-center justify-center rounded-lg transition-colors ${d ? "text-slate-400 hover:bg-slate-800" : "text-slate-500 hover:bg-slate-100"}`}>
                <Layers size={13} />
              </button>
              <button className={`flex h-6 w-6 items-center justify-center rounded-lg transition-colors ${d ? "text-slate-400 hover:bg-slate-800" : "text-slate-500 hover:bg-slate-100"}`}>
                <Camera size={13} />
            ...
            <div className={`flex flex-col rounded-2xl overflow-hidden shadow-xl border ${d ? "bg-slate-900/90 border-slate-800" : "bg-white/95 border-slate-100"}`}>
              <button onClick={() => mapRef.current?.zoomIn()}
                className={`flex h-10 w-10 items-center justify-center transition-colors border-b ${d ? "text-slate-300 hover:bg-slate-800 border-slate-800" : "text-slate-600 hover:bg-slate-50 border-slate-100"}`}>
                <Plus size={15} />
              </button>
              <button onClick={() => mapRef.current?.zoomOut()}
                className={`flex h-10 w-10 items-center justify-center transition-colors ${d ? "text-slate-300 hover:bg-slate-800" : "text-slate-600 hover:bg-slate-50"}`}>
                <Minus size={15} />
              </button>
            </div>

            <button onClick={handleNearMe}
              className={`flex h-10 w-10 items-center justify-center rounded-2xl shadow-xl border transition-colors ${nearMeActive ? "bg-indigo-600 border-indigo-500 text-white" : d ? "bg-indigo-600/90 border-indigo-500/50 text-white hover:bg-indigo-600" : "bg-indigo-600 border-indigo-500 text-white hover:bg-indigo-700"}`}>
              <LocateFixed size={15} />
            </button>
          </div>
        </div>
        ...
        {/* Status legend */}
        <div className={`absolute bottom-4 left-4 z-[500] rounded-2xl px-3 py-2.5 shadow-xl border ${d ? "bg-slate-900/90 border-slate-800" : "bg-white/95 border-slate-100"}`}>
          <p className={`mb-1.5 text-xs font-bold uppercase tracking-wider ${d ? "text-slate-600" : "text-slate-400"}`}>Status</p>
          {[["#22c55e", "Open"], ["#ef4444", "Closed"], ["#f97316", "Limited"]].map(([color, label]) => (
            <div key={label} className="flex items-center gap-2 mb-1">
              <span className="h-3 w-3 rounded-full shrink-0" style={{ background: color }} />
              <span className={`text-xs font-medium ${d ? "text-slate-400" : "text-slate-600"}`}>{label}</span>
            </div>
          ))}
        </div>

        {/* Bottom info bar */}
        <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-[500] flex items-center gap-2.5 rounded-2xl px-4 py-2.5 shadow-xl border ${d ? "bg-slate-900/95 border-slate-800" : "bg-white/95 border-slate-100"}`}>
          <Route size={14} className="text-indigo-500" />
          <span className={`text-xs font-semibold ${d ? "text-slate-300" : "text-slate-600"}`}>
            {nearMeActive ? `Within ${NEAR_ME_RADIUS_KM} km` : "PC Store Locator"}
          </span>
          <span className={`h-3 w-px ${d ? "bg-slate-700" : "bg-slate-200"}`} />
          <span className="text-xs font-bold text-indigo-500">
            {filtered.filter(p => p.open && p.availablePCs > 0).length} open & available
          </span>
          <span className={`h-3 w-px ${d ? "bg-slate-700" : "bg-slate-200"}`} />
          <span className={`text-xs ${d ? "text-slate-500" : "text-slate-400"}`}>{filtered.length} total</span>
        </div>
