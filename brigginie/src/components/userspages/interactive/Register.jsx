...
import { authAPI, storeAuthData, clearAuthData, getUser } from '../../../services/api';
...
function Register() {
  ...
  const [formData, setFormData] = useState({ firstName: '',lastName: '',email: '',password: '',confirmPassword: '', userType: '', });
  ...
  const handleSocialRegister = (provider) => {
    setError(`${provider} sign-up is not available yet. Please create your account with email.`);
  };
  ...
  const handleVerificationSuccess = (data) => {
    if (!data?.access || !data?.user) {
      setError('Verification succeeded but login data was missing.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {/* ========== REGISTRATION FORM ========== */}
      <main className="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-orange-900 px-4 py-12">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/10">
          <div className="bg-orange-500 px-8 py-7 text-center">
            ...
            <p className="text-orange-100 mt-1 text-sm">Join Rigginie PH and start building your setup.</p>
          </div>

          <div className="px-8 pt-6">
            <div className="grid grid-cols-2 rounded-lg bg-gray-100 p-1" role="tablist" aria-label="Account access">
              <button type="button" role="tab" aria-selected="false" onClick={() => navigate('/login')} className="rounded-md py-2 text-sm font-semibold text-gray-500 hover:bg-white/70 hover:text-orange-600 transition-all">
                Login
              </button>
          ...
          <form onSubmit={handleRegister} className="px-8 py-8 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-1">
              <button type="button" onClick={() => handleSocialRegister('Google')} className="flex items-center justify-center gap-2 h-12 border border-gray-200 bg-white text-gray-700 font-semibold rounded-xl shadow-sm hover:-translate-y-0.5 hover:border-gray-300 hover:shadow-md focus:ring-4 focus:ring-orange-500/10 focus:outline-none transition-all">
                <FaGoogle className="text-[#4285F4]" aria-hidden="true" />
                <span>Google / Gmail</span>
              </button>
              <button type="button" onClick={() => handleSocialRegister('Apple')} className="flex items-center justify-center gap-2 h-12 bg-gray-950 text-white font-semibold rounded-xl shadow-sm hover:-translate-y-0.5 hover:bg-black hover:shadow-md focus:ring-4 focus:ring-orange-500/10 focus:outline-none transition-all">
                <FaApple aria-hidden="true" />
                <span>Apple account</span>
              </button>
            </div>

            <div className="flex items-center gap-3 text-xs text-gray-400 uppercase tracking-wider py-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text"name="firstName"value={formData.firstName}onChange={handleChange}placeholder="Juan"className={inputClassName}required/>
              </div>
              <div>
            ...
            <div>
              ...
              <select name="userType" value={formData.userType} onChange={handleChange} className={`${inputClassName} bg-white cursor-pointer`} required>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-orange-500 hover:bg-orange-600 hover:-translate-y-0.5 disabled:bg-orange-300 disabled:hover:translate-y-0 text-white font-bold rounded-xl shadow-lg shadow-orange-500/20 transition-all flex items-center justify-center gap-2 mt-2"
            >
              {loading ? (
