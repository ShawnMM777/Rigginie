...
import { productAPI, authAPI, getUser } from "../../../services/api";
...
const HARDWARE = {
    ROUTE: 'Router ', SWTICH: 'Switches', MODEM: 'Modems', HUBS: 'HUBS', MBD: 'Motherboard',
};

function NETWORKING({menu, label}) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col" >
            <nav className="w-full bg-black py-1 px-2 flex justify-end items-center gap-x-3">
                <a onClick={() => navigate('/forum')} className="font-medium hover:underline text-amber-100 cursor-pointer">Forum</a>
            ...
            <nav className="w-full bg-orange-500 py-4 px-7 md:px-10 shadow-md sticky top-0 z-40">
                <div className="max-w-7xl mx-auto flex flex-col gap-4">
                    <div className="flex items-center gap-4 md:gap-6">
                        ...
                        <button onClick={() => setIsNavOpen(!isNavOpen)} className="md:hidden ml-auto text-white text-4xl focus:outline-none">
                            {isNavOpen ? '✕' : '☰'}
                        </button>
                    </div>
                    <div className="hidden md:flex items-center justify-between text-white font-serif font-semibold text-sm lg:text-base">
                        <Link to="/pc-stores" className="hover:text-orange-200 transition-colors px-2 py-1">PC STORE</Link>
                        <Link to="/laptops" className="hover:text-orange-200 transition-colors px-2 py-1">LAPTOPS</Link>
                        <Link to="/components" className="hover:text-orange-200 transition-colors px-2 py-1">NETWORKING</Link>
                        <Link to="/servers" className="hover:text-orange-200 transition-colors px-2 py-1">SERVERS</Link>
                        <Link to="/ai-build" className="hover:text-orange-200 transition-colors px-2 py-1">AI BUILD</Link>
                    </div>
                </div>
                ...
                {isNavOpen && (
                    <div className="md:hidden mt-6 pt-6 border-t border-orange-400 bg-orange-500">
                        ...
                        <div className="flex flex-col gap-4 text-white font-serif font-semibold text-center py-4">
...
function ProductCard({ product, onCartUpdated, onError }) {
    ...
    const navigate = useNavigate();
    const img = product.img1 ? (product.img1.startsWith('http') ? product.img1 : `http://localhost:8000${product.img1}`) : null;
